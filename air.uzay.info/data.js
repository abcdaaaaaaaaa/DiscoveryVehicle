...
const SensorValue = round4(limit((toplam2 / (Math.pow(2, 12) - 1)), 0, 1)); // analogRead()
const temperature = round4(limit(((toplam3 / 10) - 140), -10, 50)); // The IOT system you are using may be having problems with negative numbers.
const rh = round4(limit(((toplam4 / 10) - 100), 0, 100)); // The IOT system you are using may be having problems with negative numbers.
const SensorValue2 = round4(limit((toplam5 / (Math.pow(2, 12) - 1)), 0, 1)); // analogRead()
...
gas_params.forEach((gas, i) => {
    ...
    if (standard == 1) {
        ...
        lastppmvalue = Math.round(limit(Sensorppm(adjusted_valuea, adjusted_valueb, SensorValue, CorrectionCoefficient(scaledTemperature, rh)), 1, maxair * CorrectionCoefficient(scaledTemperature, rh)));
    } else {
    ...
    document.cookie = `value${i+4}=${globalThis[`ppmvalue${i+1}`]}`;
